...
<body>
	...
	<%
	PreparedStatement tt2 = con.prepareStatement(
			"select count(*) from `booking_receipt`,`trek_event`,treker_user where treker_user.uid=booking_receipt.uid and trek_event.eid=booking_receipt.eid and t_moible=? and booking_receipt.eid=? ");
	tt2.setInt(2, eid);
	tt2.setString(1, un);
	...
	PreparedStatement tt = con.prepareStatement(
			"INSERT INTO `booking_receipt`(`uid`, `eid`, `r_amount`, `rdate`, `b_status`) VALUES(?,?,?,?,?)");
	//tt.setInt(1,no);
	tt.setInt(1, uid);
	tt.setInt(2, eid);
	tt.setDouble(3, amt);
